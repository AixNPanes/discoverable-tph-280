...
from ha_mqtt_discoverable import Settings, mqtt as MQTT
...
class Hygrometer(Guage):
    ...
    def __init__(cls, mqtt:Settings.MQTT, name:str='Hygrometer', \
            device_class='humidity'):
        super(Hygrometer, cls).__init__(mqtt=mqtt, name=name, \
                device_class=device_class, info_class=HygrometerInfo,
                callback=Hygrometer.command_callback)

    def set_units(cls, unit:str):
        cls.units = HumidityeUnits.units(unit)

    @staticmethod
    def command_callback(client: Client, user_data, message: MQTTMessage):
        callback_payload = message.payload.decode()
        logging.info(f'Hygrometer received {callback_payload} from HA')
